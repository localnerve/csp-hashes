...
/**
 * cspHashes
 * Accepts an algorithm and content buffer/string and returns a hashes object
 * containing all CSP hashes for inline scripts and styles. This is useful for
 * users who just have a few files to process and will handle I/O themselves,
 * without needing a stream.
 *
 * @param {Buffer|String} content - The HTML content to extract hashes from.
 * @param {String} [algo] - hash algorithm, default sha256. Can be sha384, sha512.
 * @returns {Object} The hashes object with script and style elements and attributes.
 */
export function cspHashes (content, algo = 'sha256') {
...
/**
 * hashstream
 * Accepts the processing options and returns a Transform object stream.
 *
 ...
 * @returns Transform object stream that accepts Vinyl or plain file-like objects with `path` and `contents`.
 */
export function hashstream ({
  ...
  const transformObjectStream = new Transform({
    ...
    transform: (file, enc, done) => {
      ...
      const hashes = cspHashes(content, algo);
